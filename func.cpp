...
void readAccount(string a) {
	...
	while (!f.eof()) {
		...
		hoten = e + " " + d;
		if (a == c) {
			cout << "Ma so sinh vien: " << c << endl;
			cout << "Ho va ten: " << hoten << endl;
			cout << "Gioi tinh: " << g << endl;
			cout << "Ngay sinh: " << h << endl;
			cout << "ID: " << i << endl;
			break;
		}
	}
...
void readCourseAccount(string a, SVList& svlist) {
	...
	while (temp != NULL) {
		if (a == temp->MSSV) {
			...
			f1.open("CoursesList.csv", ios::in);
			if (f1.fail()) {
...
void luuSVList(SVList& svlist,string a) {
	fstream f;
	f.open(a, ios::in);
	if (f.fail()) {
		cout << "Can't open this file";
...
void menuGV(string a, SVList& svlist, int n, AccountList& accountlistGV) {
	...
	do {
		...
		case 4:
			...
			if (n == 1) {
				cin.ignore();
				menuThaoTacCacKhoaHoc(courselist,svlist);
			}
			break;
		case 5:
			system("cls");
...
void menuThaoTacCacKhoaHoc(CourseList& courselist,SVList& svlist) {
	int luachon;
	do {
		system("cls");
		cout << "1. Xem danh sach hoc vien cua mot khoa hoc" << endl;
		cout << "2. Them khoa hoc" << endl;
		cout << "3. Cap nhat thong tin khoa hoc" << endl;
		cout << "4. Them mot hoc vien vao khoa hoc" << endl;
		cout << "5. Xoa mot hoc vien ra khoi khoa hoc" << endl;
		cout << "6. Xoa khoa hoc" << endl;
		cout << "7. Thoat" << endl;
		cout << endl;
		cout << "Lua chon cua ban: ";
		cin >> luachon;
		...
		case 4:
			system("cls");
			them1SVKhoaHoc(svlist);
			luuMonHocSVvaoFile(svlist);
			system("pause");
			break;
		case 5:
			system("cls");
			xoa1SVKhoaHoc(svlist);
			luuMonHocSVvaoFile(svlist);
			system("pause");
			break;
		case 6:
			system("cls");
			xoaKhoaHoc(courselist);
			system("pause");
			break;
		case 7:
			break;
...
void themHocSinhVaoKhoaHocBangFile(string a, string b) {
	...
	getline(f2, dongdau);
	string No, Maso, Ten, Ho, GioiTinh, Ngaysinh, ID;
	f1 << "STT,MSSV,Ten,Ho,Gioi tinh,Ngay sinh,ID,DiemGK,DiemCK,DiemCong,GPA" << endl;
	while (!f2.eof()) {
		getline(f2, No, ',');
		if (No == "")
			return;
...
void capnhatThongTinKhoaHoc(CourseList& courselist) {
	...
	f << "MaMH,TenMH,Lop,TenGV,So tin,Sluong,Nam hoc,Thoi gian" << endl;
	i = courselist.Head;
	while (i != NULL) {
...
void them1SVKhoaHoc(SVList& svlist) {
	cin.ignore();
	string a;
	cout << "Nhap ma khoa hoc: ";
	getline(cin, a);
	...
	f.close();
	themMonHocSV(svlist, Maso, a);
}

//HAM NAY THEM MON HOC VAO SVLIST DE SINH VIEN CO THE THAY MON HOC CUA MINH
//a la MSSV, b la ma mon hoc
void themMonHocSV(SVList& svlist,string a,string b) {
	SinhVien* temp = svlist.Head;
	while (temp != NULL) {
		if (a == temp->MSSV) {
			if (temp->data.Head == NULL) {
...
void luuMonHocSVvaoFile(SVList svlist) {
	fstream f;
...
void xoa1SVKhoaHoc(SVList& svlist) {
	cin.ignore();
	string a, b, c;
	cout << "Nhap ma khoa hoc: ";
	getline(cin, a);
	...
	fstream f;
	c = "Courses\\" + a + ".csv";
	SVList svlistkhoahoc;
	khoiTaoSVList(svlistkhoahoc);
	luuSVList(svlistkhoahoc, c);
	SinhVien* i = svlistkhoahoc.Head;
	while (i != NULL) {
		if (i->MSSV == b)
			break;
		i = i->pnext;
	...
	if (i == NULL)
		return;
	if (i == svlistkhoahoc.Head) {
		svlistkhoahoc.Head = svlistkhoahoc.Head->pnext;
		delete i;
	}
	else {
		SinhVien* j = svlistkhoahoc.Head;
		while (j->pnext != i)
			j = j->pnext;
		j->pnext = i->pnext;
		delete i;
	...
	f << "No,MSSV,Ten,Ho,Gioitinh,Ngaysinh,ID,DiemGK,DiemCK,DiemCong,GPA" << endl;
	i = svlistkhoahoc.Head;
	int dem = 1;
	while (i != NULL) {
		f << dem << "," << i->MSSV << "," << i->Ten << "," << i->Ho << "," << i->Gioitinh << "," << i->Ngaysinh << "," << i->ID;
		if (i->pnext != NULL)
			f << endl;
		else
			break;
	...
	f.close();
	xoaMonHocSV(svlist, b, a);
}

//HAM NAY XOA MON HOC CUA SINH VIEN KHOI SVLIST DE SINH VIEN KHONG THE THAY MON HOC CUA MINH
//a la MSSV, b la ma mon hoc
void xoaMonHocSV(SVList& svlist, string a, string b) {
	SinhVien* temp = svlist.Head;
	while (temp != NULL) {
		if (temp->MSSV == a) {
			Course* temp1 = temp->data.Head;
			if (temp1->maMH == b) {
				temp->data.Head = temp->data.Head->pnext;
				delete temp1;
				break;
			}
			else {
				while (temp1->pnext != NULL) {
					if (temp1->pnext->maMH == b) {
						Course* temp2 = temp1->pnext;
...
void xoaKhoaHoc(CourseList& courselist) {
	cin.ignore();
	string a;
	...
	f << "MaMH,TenMH,TenLop,TenGV,SoTin,SoLuongMax,NamHoc,GioHoc" << endl;
	i = courselist.Head;
	while (i != NULL) {
...
void xuatHocSinhMonHoc() {
	...
	string dongdau;
	getline(f, dongdau);
	cout << left << setw(10) << "STT" << setw(15) << "MSSV" << setw(30) << "Ho va ten" << setw(15) << "Gioi tinh" << setw(20) << "Ngay sinh" << setw(20) << "ID" << setw(10) << "Diem GK" << setw(10) << "Diem CK" << setw(10) << "Diem Cong" << setw(10) << "GPA" << endl;
	string No, Maso, Ten, Ho, GioiTinh, Ngaysinh, ID;
	while (!f.eof()) {
		getline(f, No, ',');
		if (No == "")
